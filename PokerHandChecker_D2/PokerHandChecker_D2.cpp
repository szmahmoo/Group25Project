/**********************************************************************************************
 * 
 * PokerHandChecker_D2.cpp
 * COSC 1430 | Group 25 Project
 *
 * D2: Second Draft of Program
 *      - Added Missing: bools for hand cases and ranking of hands, winner decision
 *      - Need to do further case testing to check if winner declaration is correct
 *      - Found: Wrong when dealing with Full House, Must fix
 *
...
/* Poker Hand Cases dealing with Rank
 * Cases:   High Card, One Pair, Two Pairs, Three of a Kind, Straight, Flush, Full House,
 *          Four of a Kind, Straight Flush, Royal Flush
 */
...
string getType(Hand hand)
{
    ...
    if(isPair(hand) && ! isTwoPairs(hand))
    {
    ...
    else if(isThreeOfKind(hand))
    {
        ...
        ss << "3 of Kind of " << getFaceName(rank);
    }
    ...
    else if(isFourOfKind(hand))
    {
        ...
        ss << "4 of a Kind of " << getFaceName(rank);
    }
    else if(isFlush(hand) && isStraight(hand))
    {
        ss << "Straight Flush";
